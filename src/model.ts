...
export function previousExercises(e:Exercise,history:Session[],before=Infinity){
 return history.filter(s=>s.finishedAt&&s.finishedAt<=before).sort((a,b)=>b.finishedAt!-a.finishedAt!).flatMap(s=>s.exercises).filter(x=>sameExercise(x.exercise,e));
}
...
export function progressionSuggestions(session:Session|null,history:Session[]):ProgressionSuggestion[]{
 ...
 return session.exercises.flatMap((entry,exerciseIndex)=>{
  ...
  const recent=history.filter(s=>s.id!==session.id&&s.name===session.name&&s.finishedAt&&s.finishedAt<=session.startedAt).sort((a,b)=>b.finishedAt!-a.finishedAt!).slice(0,2);
